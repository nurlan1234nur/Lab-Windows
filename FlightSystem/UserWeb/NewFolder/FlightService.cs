...
using UserWeb.Components.Pages;
...
namespace UserWeb.Components.Pages
{
    public class FlightApiClient
    {
        ...
        {
            Console.Write(origin, date);
            var response = await _http.GetFromJsonAsync<List<Flight>>(
                $"api/flight/flights?origin={origin}&destination={destination}&date={date:yyyy-MM-dd}");
